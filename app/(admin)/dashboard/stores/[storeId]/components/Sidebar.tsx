...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, storeId }) => {
  return (
    <>
      {/* Overlay */}
      {isOpen && (
      ...
      >
        <div className="flex flex-col min-h-screen">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <Link href={`/dashboard/stores/${storeId}/discounts`}>
              <SidebarItem icon={<span>📜</span>} text="Discounts" />
            </Link>
            <Link href={`/dashboard/stores/${storeId}/journals`}>
              <SidebarItem icon={<span>👥</span>} text="Journal" />
            </Link>
            <Link href={`/dashboard/stores/${storeId}/orders`}>
              <SidebarItem icon={<span>👥</span>} text="Orders" />
            </Link>
            <Link href={`/dashboard/stores/${storeId}/payments`}>
              <SidebarItem icon={<span>📜</span>} text="Payments" />
            </Link>
          </div>
        </div>
      </div>
